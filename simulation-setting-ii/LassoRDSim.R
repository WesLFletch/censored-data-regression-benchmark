...
lasso_rds = function(new_df, true_tte, true_sig_preds){
  ...
  start_time = Sys.time()
  beta_hat = cv.glmnet(as.matrix(train_df[,-1]), train_df$surv_time,
...
LASSO = function(new_df, true_tte, true_sig_preds){
  lasso_rds(new_df, true_tte, true_sig_preds)
}
