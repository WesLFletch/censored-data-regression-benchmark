...
coxboost_rds = function(new_df, true_tte, true_sig_preds){
  ...
  start_time = Sys.time()
  beta_hat = coef(
    CoxBoost(train_df$surv_time[,1], train_df$surv_time[,2], as.matrix(train_df[,-1]))
  )
...
CB = function(new_df, true_tte, true_sig_preds){
  coxboost_rds(new_df, true_tte, true_sig_preds)
}
