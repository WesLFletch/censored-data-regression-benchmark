...
bhprocedure_rds = function(new_df, true_tte, true_sig_preds){
  ...
  sig_preds = colnames(train_df)[1+which(BH(pvals, alpha = 0.05, silent = TRUE)$rejected)]
  ph_model = coxph(
    surv_time ~ .,
...
BHP = function(new_df, true_tte, true_sig_preds){
  bhprocedure_rds(new_df, true_tte, true_sig_preds)
}
