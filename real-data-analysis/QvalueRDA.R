...
qvalue_rda = function(full_df, fold_idxs, prelim_mrna){
  ...
  }) %>% replace_na(., 1) %>% qvalue(., fdr.level = 0.05))[[6]]) %>%
    colnames(full_df)[.]
...
QV = function(full_df, fold_idxs, prelim_mrna){
  qvalue_rda(full_df, fold_idxs, prelim_mrna)
}
