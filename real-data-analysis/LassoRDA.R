...
lasso_rda = function(full_df, fold_idxs, prelim_mrna){
  ...
  beta_hat =
    cv.glmnet(x = as.matrix(full_df[,-(1:4)]), y = full_df$surv_time, family = "cox",
              type.measure = "C")$lambda.min %>%
    glmnet(x = full_df[,-(1:4)], y = full_df$surv_time, family = "cox", lambda = .) %>%
    coef(.) %>%
    as.numeric(.)
  # beta_hat =
  #   glmnet(x = full_df[,-(1:4)], y = full_df$surv_time, family = "cox", lambda = 0.1) %>%
  #   coef(.) %>%
  #   as.numeric(.)
  ...
  run_fold = function(full_df, test_idxs){
    ...
    beta_hat = cv.glmnet(x = as.matrix(train_df[,-(1:2)]), y = train_df$surv_time,
                         penalty.factor = c(rep(0, 2), rep(1, ncol(train_df)-4)),
                         family = "cox", type.measure = "C")$lambda.min %>%
...
LASSO = function(full_df, fold_idxs, prelim_mrna){
  lasso_rda(full_df, fold_idxs, prelim_mrna)
}
